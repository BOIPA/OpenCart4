<?php
namespace Opencart\Catalog\Controller\Extension\Boipa\Payment;
require_once DIR_EXTENSION.'boipa/system/library/boipa/payments.php';
class Boipa extends \Opencart\System\Engine\Controller {
	...
	public function index() {
		...
		return $this->load->view('extension/boipa/payment/boipa', $data);
	}
		...
		if (!$json) {
			...
			if ($order_info = $this->model_checkout_order->getOrder($this->session->data['order_id'])) {
				...
				try {
					...
					$payment_boipa_pay_action = $this->config->get('payment_boipa_pay_action'); //1 refer to auth, 0 refer to purchase
					if ($payment_boipa_pay_action){
					...
					if(isset($res->result) && $res->result == 'success'){
						...
						$data['merchantId'] = trim($this->config->get('payment_boipa_clientid'));
						$testmode = $this->config->get('payment_boipa_testmode');
						if ($testmode){
							$data['action'] = $this->config->get('payment_boipa_test_cashier_url');
							$data['java_script_url'] = $this->config->get('payment_boipa_test_javascript_url');
						}else{
							$data['action'] = $this->config->get('payment_boipa_cashier_url');
							$data['java_script_url'] = $this->config->get('payment_boipa_javascript_url');
						}
						...
						$this->model_extension_boipa_payment_boipa->addOrder($gateway_data);
						if($this->config->get('payment_boipa_pay_type') == 1){
							//iframe payment mode
							$json['action'] = str_replace('&amp;', '&', $this->url->link('extension/boipa/payment/boipa|iframe', 'language=' . $this->config->get('config_language')));
						}else if($this->config->get('payment_boipa_pay_type') == 2){
						...
						$json['merchantId'] = trim($this->config->get('payment_boipa_clientid'));
						$this->cart->clear();
	...
	private function prepareData($order_info){
		...
		$post_data['merchantLandingPageUrl'] = str_replace('&amp;', '&', $this->url->link('extension/boipa/payment/boipa|redirectBack', 'order_id='.$this->session->data['order_id'].'&language=' . $this->config->get('config_language')));
		$post_data['merchantNotificationUrl'] = str_replace('&amp;', '&', $this->url->link('extension/boipa/payment/boipa|callback', 'order_id='.$this->session->data['order_id'].'&language=' . $this->config->get('config_language')));
	...
	public function iframe(){
        ...
        if ($this->config->get('payment_boipa_pay_type') != self::PAYMENT_IFRAME) {
			$data['errors']['error_payment_method'] = $this->language->get('error_payment_method');
        }
		...
		$testmode = $this->config->get('payment_boipa_testmode');
		if ($testmode){
	        $data['baseUrl'] = $this->config->get('payment_boipa_test_cashier_url');
			$data['java_script_url'] = $this->config->get('payment_boipa_test_javascript_url');
	    }else{
	        $data['baseUrl'] = $this->config->get('payment_boipa_cashier_url');
			$data['java_script_url'] = $this->config->get('payment_boipa_javascript_url');
	    }
		...
		$this->response->setOutput($this->load->view('extension/boipa/payment/iframe', $data));
	}
	// init the SDK configuration settings
	private function initConfig(){
	    $this->environment_params['merchantId'] =  trim($this->config->get('payment_boipa_clientid'));
	    $this->environment_params['password'] = trim($this->config->get('payment_boipa_password'));
	    $testmode = $this->config->get('payment_boipa_testmode');
	    if ($testmode){
	        $this->environment_params['tokenURL'] = $this->config->get('payment_boipa_test_token_url');
	        $this->environment_params['paymentsURL'] = $this->config->get('payment_boipa_test_payments_url');
	        $this->environment_params['baseUrl'] = $this->config->get('payment_boipa_test_cashier_url');
	        $this->environment_params['jsApiUrl'] = $this->config->get('payment_boipa_test_javascript_url');
	    }else{
	        $this->environment_params['tokenURL'] = $this->config->get('payment_boipa_token_url');
	        $this->environment_params['paymentsURL'] = $this->config->get('payment_boipa_payments_url');
	        $this->environment_params['baseUrl'] = $this->config->get('payment_boipa_cashier_url');
	        $this->environment_params['jsApiUrl'] = $this->config->get('payment_boipa_javascript_url');
	    }
	}
	...
	private function checkStatus($paymentData,$option){
		$boipa_order_id = $paymentData['boipa_order_id'];
	    ...
	    try {
	        ...
	        if(!isset($result->result) || $result->result != 'success'){
				...
				$this->model_checkout_order->addHistory($order_id, $this->config->get('payment_boipa_failed_status_id'),'Failed',true);
				return false;
	        }else{
	            if($result->status == 'SET_FOR_CAPTURE' || $result->status == 'CAPTURED'){
					if($option == $this::CHECKSTATUS_CALLBACK){
						//PURCHASE was successful
						$this->model_extension_boipa_payment_boipa->updatePaymentData($boipa_order_id,static::PURCHASED_STATUS);
						$this->model_extension_boipa_payment_boipa->addTransaction($boipa_order_id,'payment', $amount);
						$this->model_checkout_order->addHistory($order_id, $this->config->get('payment_boipa_success_status_id'),'Paid',true);
					}
					return true;
	            }else if($result->status == 'NOT_SET_FOR_CAPTURE'){
					if($option == $this::CHECKSTATUS_CALLBACK){
						// AUTH was successful
						$this->model_extension_boipa_payment_boipa->updatePaymentData($boipa_order_id,static::AUTHORIZED_STATUS);
						$this->model_extension_boipa_payment_boipa->addTransaction($boipa_order_id,'auth', $amount);
						$this->model_checkout_order->addHistory($order_id, $this->config->get('payment_boipa_auth_status_id'),'Authorized',true);
					}
					return true;
	            }else{
	...
	public function redirectBack(){
	    ...
	    if(!isset($this->request->get['merchantTxId']) || !isset($this->request->get['order_id'])){
			...
			$paymentData = $this->model_extension_boipa_payment_boipa->getPaymentData($order_id,$merchantTxId);
